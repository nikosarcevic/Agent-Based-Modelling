...
for i = 1:n_particles
    for j = 2:timesteps
        ...
        if abs(position(1, i, j)) > per_bound     % particle left (<) or right (>) of box
               position(1, i, j) = position(1, i, j) - 2 * sign(position(1, i, j)) * per_bound;
        elseif abs(position(2, i,j )) > per_bound % particle below (<) or above (>) of box
               position(2, i, j) = position(2, i, j) - 2 * sign(position(2, i, j)) * per_bound;
        end
    end    
end
