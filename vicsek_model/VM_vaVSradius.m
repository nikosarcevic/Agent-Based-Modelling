...
for i = 1:n_sims % loop over the number of simulations
    noise = 1; % set noise to a constant value
	v_a(1, i) = r; % get the interacting radius values
	v_a(2, i) = vicsek(n_particles, dt, t_max, v, per_bound, L, r, noise); % get alignment values
    r = r + 0.1; % increase the interacting radius by .1 in each iteration
    
end

% Plot the aligment angle v_a as a function of the increasing radius:
plt = plot(v_a(1, :),v_a(2, :));
...
% Axes label settings:
x_ax = xlabel("Interacting radius $r$"); % x label
...
yline1.LabelHorizontalAlignment = "right";
yline2.LabelHorizontalAlignment = "right";

box on % include the plot box
grid on % plot the grid
%grid minor
axis square % keep the axes square
saveas(gcf,'VM_vaVSradius.svg')

% Define a Vicsek model and return an alignment angle v_a
...
c_gray2 = "#DCDCDC";
    ...
    for time = 1:t_max % loop from t_initial to t_max
            ...
            v_a = (v_mag/(n_particles * v)); % alignment angle (avg of all velocity vectors of all agents)
