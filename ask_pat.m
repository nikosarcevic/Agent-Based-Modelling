...
n_sims = t_max; %number of simulations
...
for i = 1:n_sims % loop over the number of simulations
    ...
    noise = noise + 0.5; % increase the noise by .1 in each iteration
...
c_gray2 = "#DCDCDC";
    ...
    for time = 1:t_max % loop from t_initial to t_max
            ...
            writerObj = VideoWriter(strcat("111TEST.avi")); %set the name of your video
            ...
            stitle = subtitle({strcat("$\rho$ = " + num2str(rho) + ...
                                      ", $r$ = " + num2str(r) + ...
                                      ", $\eta_i$ = " + num2str(noise_i(time)) + ...
                                      ", $\mathrm{d}t$ = " + num2str(dt) + ...
                                      ", $t_\mathrm{max}$ = " + num2str(t_max)) + ...
                                      ", run = " + num2str(step(time))}); % subtitle with preferred information params
            ...
            % save quiver plot figs as svg for each time iteration
            %saveas(q, sprintf("q2quivernoise%d.svg", time))
            %Close video writer:
    close(writerObj);
